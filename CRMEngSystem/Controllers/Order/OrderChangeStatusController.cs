        ...
        public async Task<IActionResult> OrderChangeStatus(int EntityId, OrderStatusValue Status)
        {
            ...
            switch (Status)
            {
                case OrderStatusValue.Processing:
                    order!.DateTimeProcessingStatusStart = DateTime.Now;
                    break;
                case OrderStatusValue.Offer:
                    order!.DateTimeOfferStatusStart = DateTime.Now;
                    break;
                case OrderStatusValue.Execution:
                    order!.DateTimeExecutionStatusStart = DateTime.Now;
                    break;
                case OrderStatusValue.Completed:
                    order!.DateTimeCompletedStatusStart = DateTime.Now;
                    break;
            }
            order!.Status = Status;
